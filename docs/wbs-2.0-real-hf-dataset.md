...
| P19 Real HF N-best Extraction | blocked | `wbs/v2/P19-real-hf-nbest-extraction-gpu` |
...
environment에는 없다. CPU generation은 validation 1.72초 chunk 1개도 120초 이상 완료되지 않아 중단했다.
이후 CUDA runtime에서 RTX 4080 SUPER 16GB GPU를 확인하고 재시도했지만,
`openai/whisper-large-v3` beam 20 / return 20은 CUDA OOM으로 실패했다. fp16 model loading과 실제
attention mask 전달을 적용해도 full validation extraction은 artifact 0개 상태에서 실패했다. 따라서 P19는
available GPU가 있어도 current model/decode setting에서는 hardware/runtime blocker로 멈춘다.
...
- `docs/reports/experiments/p19_gpu_beam20_stop_report.md`
